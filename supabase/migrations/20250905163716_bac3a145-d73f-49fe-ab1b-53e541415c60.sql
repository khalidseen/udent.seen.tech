-- إنشاء فهارس لتحسين أداء قاعدة البيانات (بدون CONCURRENTLY للمايجريشن)
...
CREATE INDEX IF NOT EXISTS idx_appointments_status_date ON public.appointments(status, appointment_date) WHERE status IN ('scheduled', 'confirmed');
CREATE INDEX IF NOT EXISTS idx_appointments_upcoming ON public.appointments(clinic_id, appointment_date) WHERE appointment_date >= CURRENT_DATE AND status IN ('scheduled', 'confirmed');
...
CREATE INDEX IF NOT EXISTS idx_invoices_balance_due ON public.invoices(balance_due) WHERE balance_due > 0;
...
CREATE INDEX IF NOT EXISTS idx_supplies_low_stock ON public.medical_supplies(clinic_id) WHERE current_stock <= minimum_stock AND is_active = true;
CREATE INDEX IF NOT EXISTS idx_supplies_expiry_alert ON public.medical_supplies(clinic_id, expiry_date) WHERE expiry_date IS NOT NULL AND expiry_date <= CURRENT_DATE + INTERVAL '30 days' AND is_active = true;
CREATE INDEX IF NOT EXISTS idx_supplies_category ON public.medical_supplies(category, clinic_id) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_prescriptions_status ON public.prescriptions(status, clinic_id);
...
CREATE INDEX IF NOT EXISTS idx_security_events_risk_score ON public.security_events(risk_score DESC) WHERE risk_score >= 50;
CREATE INDEX IF NOT EXISTS idx_security_events_category ON public.security_events(event_category, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_purchase_orders_supplier ON public.purchase_orders(supplier, clinic_id);
