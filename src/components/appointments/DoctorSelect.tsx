...
const DoctorSelect = ({ doctors, value, onValueChange, label = "الطبيب المعالج", required = true, allowNone = false, className }: DoctorSelectProps) => {
  return (
    <div className={`space-y-2 ${className || ''}`}>
      ...
      <Select value={value} onValueChange={onValueChange}>
        ...
        <SelectContent>
          {allowNone && <SelectItem value="none">بدون طبيب محدد</SelectItem>}
          {doctors?.map((doctor) => (
